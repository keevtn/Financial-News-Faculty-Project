...
export default function TabNav({ active, onChange }: TabNavProps) {
  return (
    <div className="bg-[#0a0f1e] border-b border-[#1e2d4a] px-6 flex items-end gap-0 shrink-0">
      {TABS.map((tab) => {
        ...
        return (
          ...
          >
            {tab.label}
            <span className={`text-[10px] font-normal ${isActive ? "text-slate-400" : "text-slate-600"}`}>
              {tab.sub}
            </span>
          </button>
        );
